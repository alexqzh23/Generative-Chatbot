...
class LeeOscillator():
    ...
    def __init__(self, a = [1, 1, 1, 1, -1, -1, -1, -1], b = [0.6, 0.6, -0.5, 0.5, -0.6, -0.6, -0.5, 0.5], K = 50, N = 100):
        ...
        # self.tanh  = torch.from_numpy(pd.read_csv('./LeeOscillator-Tanh.csv').values).cuda()
        # print(self.tanh)
        # self.sigmoid = torch.from_numpy(pd.read_csv('./LeeOscillator-Sigmoid.csv').values).cuda()
        # print(self.sigmoid)
    ...
    def Tanh(self, x):
        ...
        u = torch.zeros((N, x.shape[1], x.shape[0]), dtype = torch.float32).to(x.device)
        v = torch.zeros((N, x.shape[1], x.shape[0]), dtype = torch.float32).to(x.device)
        z = torch.zeros((N, x.shape[1], x.shape[0]), dtype = torch.float32).to(x.device)
        w = 0
        u[0] = u[0] + 0.2
        z[0] = z[0] + 0.2
