...
class ChaoticLSTM(nn.Module):
    ...
    def __init__(self, inputSize, hiddenSize, chaotic = False, bidirection = False):
        ...
        self.Wi = nn.Parameter(torch.Tensor(inputSize, hiddenSize * 4)).cuda()
        # Create the parameter of the hidden.
        self.Wh = nn.Parameter(torch.Tensor(hiddenSize, hiddenSize * 4)).cuda()
        # Create the parameter of the bias.
        self.B = nn.Parameter(torch.Tensor(hiddenSize * 4)).cuda()
        # Check the number of directions.
        if bidirection == True:
            # Create the parameter of the inverse input.
            self.Winvi = nn.Parameter(torch.Tensor(inputSize, hiddenSize * 4)).cuda()
            # Create the parameter of the inverse hidden.
            self.Winvh = nn.Parameter(torch.Tensor(hiddenSize, hiddenSize * 4)).cuda()
            # Create the parameter of the inverse bias.
            self.Binv = nn.Parameter(torch.Tensor(hiddenSize * 4)).cuda()
