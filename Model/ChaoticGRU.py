...
class ChaoticGRU(nn.Module):
    ...
    def forward(self, x, initStates = None):
        ...
        # Check the direction.
        # print("In ChaoticGRU, output_1: ",  torch.tensor(output).size())
        if self.bidirection == True:
            ...
            # print("ht: ", ht.size(), 'hinvt: ', hinvt.size())
            # ht = torch.cat([ht, hinvt], dim = 1)
            ht = torch.stack([ht, hinvt], dim=0)
        # Concatenate the output.
        # print("In ChaoticGRU, output_2: ", torch.tensor(output).size())
...
if __name__ == "__main__":
    ...
    CGRU = ChaoticGRU(inputSize = 1536, hiddenSize = 1536, chaotic = False, bidirection = True)
    # Test the Bi-GRU.
    x = torch.rand([10, 128, 1536])
    print(x.shape)
    output, h = CGRU(x)
    print(output.shape)
    print(h.shape)
